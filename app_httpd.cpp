...
static const char QR_PAGE[] = R"rawhtml(<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
...
<style>
  ...
  h2{color:#e94560}
  button{background:#e94560;color:#fff;border:none;padding:12px 28px;font-size:16px;border-radius:8px;cursor:pointer;margin:10px}
  button:hover{background:#c73652}
  #result{font-size:18px;margin:16px auto;padding:14px;background:#16213e;border-radius:8px;max-width:480px;word-break:break-all;min-height:40px}
  #response{font-size:15px;margin:10px auto;padding:14px;background:#0f3460;border-radius:8px;max-width:480px;word-break:break-all;min-height:36px;display:none}
  #response.success{border:1px solid #00b894}
  #response.error{border:1px solid #e94560}
  #preview{max-width:100%;border-radius:8px;margin-top:10px;display:none}
  #status{color:#aaa;font-size:14px;margin:6px 0}
  canvas{display:none}
</style>
</head>
<body>
<h2>ESP32-CAM QR Scanner</h2>
<button onclick="scan()">Scan QR Code</button>
<button onclick="autoScan()">Auto Scan</button>
<button id="stopBtn" onclick="stopAuto()" style="background:#555;display:none">Stop</button>
<div id="status">Ready</div>
<div id="result">Point camera at a QR code and press Scan</div>
<div id="response"></div>
<img id="preview">
...
<script>
var autoTimer = null;
var captureUrl = location.origin + '/capture';
var phpUrl = 'http://192.168.43.57/smart_attendance_v2/api/attendance.php';
...
function sendToServer(id){
  setResponse('Sending to server...', true);
  fetch(phpUrl, {
    method: 'POST',
    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
    body: 'student_code=' + encodeURIComponent(id)
  })
  .then(r => r.text())
  .then(text => {
    try {
      var data = JSON.parse(text);
      setResponse(JSON.stringify(data, null, 2), true);
    } catch(e) {
      setResponse(text, true);
    }
  })
  .catch(e => { setResponse('Cannot reach PHP server at ' + phpUrl + ' — check XAMPP is running and IP is correct. (' + e + ')', false); });
}

function scan(){
  setStatus('Capturing...');
  fetch(captureUrl + '?t=' + Date.now())
    .then(r => r.blob())
    .then(blob => {
      ...
      img.onload = function(){
        ...
        if(code){
          setStatus('QR Found!');
          setResult('ID: ' + code.data);
          sendToServer(code.data);
        } else {
          setStatus('No QR code found');
          setResult('No QR code detected. Try adjusting distance/lighting.');
        }
...
function autoScan(){
  ...
  autoTimer = setInterval(scan, 2000);
  scan();
}
...
static esp_err_t qr_scan_handler(httpd_req_t *req)
{
    httpd_resp_set_type(req, "text/html");
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    return httpd_resp_send(req, QR_PAGE, strlen(QR_PAGE));
}
